...
class SignalDetector:
    ...
    def calculate_adx(self, period: int = 14) -> Tuple[pd.Series, pd.Series, pd.Series]:
        ...
        plus_dm = pd.Series(0.0, index=self.data.index)
        minus_dm = pd.Series(0.0, index=self.data.index)
    ...
    def check_volume_confirmation(self, window: int = 20) -> bool:
        """Check if current volume confirms the signal"""
        if 'volume' not in self.data.columns or len(self.data) < window:
            return True  # If no volume data, don't filter
        ...
        return pd.notna(avg_volume_current) and current_volume >= (avg_volume_current * 0.8)
    ...
    def detect_ma_crossover(
        ...
        """Detect Moving Average Crossover signals with ADX and volume confirmation"""
        ...
        if use_volume_filter and not self.check_volume_confirmation():
            return None

        # Get ADX for trend strength
        adx, plus_di, minus_di = self.calculate_adx()
        adx_current = adx.iloc[idx_current] if not adx.empty and pd.notna(adx.iloc[idx_current]) else 0
        ...
        if short_previous < long_previous and short_current > long_current:
            # Filter by ADX - strong trend needed
            if use_adx_filter and adx_current < 20:
                return None  # Trend too weak

            distance_pct = ((short_current - long_current) / long_current) * 100

            # Adjust strength based on ADX and distance
            if distance_pct > 2 and adx_current > 30:
                strength = SignalStrength.STRONG
            elif distance_pct > 1 or adx_current > 25:
            ...
            return TradingSignal(
                ...
                message=f"Golden Cross: SMA{short_period} crossed above SMA{long_period} (ADX: {adx_current:.1f})"
            )
        ...
        # Bearish crossover (Death Cross)
            ...
            return TradingSignal(
                ...
                message=f"Death Cross: SMA{short_period} crossed below SMA{long_period} (ADX: {adx_current:.1f})"
            )
    ...
    def detect_macd_signal(self, use_volume_filter: bool = True) -> Optional[TradingSignal]:
        """Detect MACD crossover signals with volume confirmation"""
        if len(self.data) < 50:
            return None
        ...
        if macd_previous < signal_previous and macd_current > signal_current:
            ...
            if abs(hist_current) > 1 and macd_current > 0:
                strength = SignalStrength.STRONG
            ...
            return TradingSignal(
                ...
                indicators={
                    ...
                    'volume_confirmed': use_volume_filter
                },
